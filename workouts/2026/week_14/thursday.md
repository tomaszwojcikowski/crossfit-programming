# Thursday (02/04/2026)

**Setup**
- Equipment: Pull-up rig, rope climbs, open wall space (wall walks), ~10 m clear floor (HSW),
  run course (~100 m)
- Timing: 60 min hard cap
- Stimulus: 30-minute gymnastics EMOM — alternating upper pulling, rope, pressing inversion,
  handstand walk, and max-effort run. Station 6 is full rest — protect it.
- Scaling A: Rope climb → 2 rope pulls from floor. Wall walks 3 → 2. HSW 5 m → wall taps x10.
- Scaling B: Hanging knee raises instead of T2B. Rope: 3 x low ring rows or lay-to-stand rope.
  Wall walks → inchworm to wall tap. Bear crawl 10 m instead of HSW.
- Top Cues: "Kick toes to bar." "Heels to wall first — then lift hips." "Tight midline on the walk."

## Warmup (10 min)
- **R1 RAISE (2:00 easy):** Jog / shuttle runs
  - 8 x Scap pull-ups on rig
  - 6 x Hollow rocks
- **R2 ACTIVATE (1:30 moderate):** Shuttle runs
  - 6 x Kip swings
  - 6 x Push-up to down-dog
- **R3 POTENTIATE (1:00 hard):** Hard shuttle runs
  - 4 x T2B singles or 6 x hanging knee raises
  - 1 x Wall walk + bear crawl back
- Rest 30s between rounds

## Post-Warmup Mobility (90s)
- Lat stretch on rig: 30s/side
- Wrist extension rocks on floor: 30s

## Movement Prep (4 min)
- 3 x T2B singles or 5 x hanging knee raises
- 1 x Rope climb or 2 x rope pulls from floor
- 1 x Wall walk + 2 s heels-to-wall hold
- 5 m HSW or 10 x wall taps

## Main Conditioning: 30' EMOM
**Every minute on the minute for 30 minutes (5 rounds):**

- **Minute 1:** 12 Toes-to-Bar
- **Minute 2:** 1 Legless Rope Climb or 2 Rope Climbs
- **Minute 3:** 3 Wall Walks
- **Minute 4:** 10 m Handstand Walk
- **Minute 5:** Max distance run
- **Minute 6:** Rest

- **Run note:** Return to your lane before Minute 6.
- **Target:** Minutes 1-4 finish in 35-45s, leaving 15-25s to reset.

## Cooldown & Debrief (4 min)
1. 90s easy walk, nasal breathing
2. Puppy dog pose (lats/shoulders): 45s
3. Wrist flexor stretch (grip + handstand volume): 30s/side
4. Bright spot: "Which station stayed smoothest under fatigue?"
