# Tuesday (24/03/2026)

**Setup**
- Equipment: Jump ropes, pull-up rig
- Timing: 60 min hard cap
- Stimulus: EMOM primer to build rhythm, then Annie benchmark — pure output test on butterfly sit-ups and double unders.
- Scaling A: Keep rep counts in EMOM. Annie: single unders (2x count) or reduce DU by 20%.
- Scaling B: Push-ups from knees, knee raises instead of T2B, singles throughout Annie.
- Top Cues: "Tight hollow on the sit-up." "Relax the wrists — let the rope do the work."

## Warmup (10 min)
- **R1 RAISE (2:00 easy):** Jog / shuttle runs
  - 30 x Single unders
  - 8 x Calf raises
- **R2 ACTIVATE (1:30 moderate):** Shuttle runs
  - 20 x Double-under attempts (or 40 singles)
  - 8 x Hollow-body rocks
- **R3 POTENTIATE (1:00 hard):** Hard shuttle runs
  - 6 x Kip swings on rig
  - 10 x Fast DU attempts or 20 singles
- Rest 30s between rounds

## Post-Warmup Mobility (90s)
- Hip flexor stretch: 45s/side
- Thoracic extension on floor: 45s

## Skill: 15 min EMOM
**Every minute, on the minute — 3 stations, rotate:**

- Min 1: 10 Push-Ups
- Min 2: 10 Toes-to-Bar
- Min 3: 30 Double Unders / 40s practice DU

(5 rounds — 15 min total)

## Movement Prep (1 min)
- 5 x Butterfly sit-ups
- 10 x Double unders — feel the rhythm before the clock

## Main Conditioning: Benchmark "ANNIE" (10 min)
**For time — 50-40-30-20-10:**

- Butterfly Sit-Ups
- Double Unders

**Time cap:** 10 min

## Cooldown & Debrief (4 min)
1. 2 min easy walk, nasal breathing
2. Seated hamstring fold: 45s
3. Supine twist: 30s/side
4. Bright spot: "I held small unbroken sets on DU."
